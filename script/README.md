...
| `build.sh` | `wails build -clean` — build de producción, limpia `build/bin` antes de compilar. Al final lista el tamaño del binario resultante. | Antes de probar el binario empaquetado, o para verificar el objetivo de <20MB tras añadir una dependencia. |
