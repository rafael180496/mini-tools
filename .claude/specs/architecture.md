...
/backend
  ...
  /agentapprove aprobación acción por acción: channel.go (socket Unix + timeout; **si no se puede preguntar, deniega**) + hook.go (re-exec como hook PreToolUse, el mecanismo que reemplazó a `--permission-prompt-tool`, que ya no existe en Claude Code 2.1.231) + mcpserver.go (servidor MCP de stdio, conservado por si otro CLI lo usa) + settings.go. Verificado de punta a punta contra el CLI real, denegando y aprobando
