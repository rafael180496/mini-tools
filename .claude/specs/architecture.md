...
/backend
  ...
  /agents       catálogo de CLIs agénticos (Claude Code, Codex, Gemini): detección en PATH + rutas habituales (~/.local/bin, npm, bun), comando por agente y variable de entorno de su API key. Sin dependencias: los agentes se EJECUTAN como programas de terminal sobre el PTY de /localterm, no se integran por API — ver el doc del paquete. La autenticación la sigue manejando cada CLI; la app solo guarda, opcional y cifrada, una API key
