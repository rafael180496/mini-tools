...
Antes de un cambio no trivial: leer `architecture.md` para el estado actual, `go-react-contract.md` si toca `app.go`, `vault-migrations.md` si toca el schema de `vault.db`, y el `SKILL.md` si toca conectores/executor/explain/theming — tienen las desviaciones reales vs. lo planeado y los bugs ya encontrados, para no repetirlos.
