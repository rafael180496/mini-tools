...
export default function EditorTabs({tabs, activeId, onSelect, onClose, onNew}: EditorTabsProps) {
    return (
        <div className="flex items-center gap-1 border-b border-neutral-800 bg-neutral-900 px-2 pt-1">
            {tabs.map((t) => (
                <div
                    key={t.id}
                    onClick={() => onSelect(t.id)}
                    className={`flex cursor-pointer items-center gap-2 rounded-t px-3 py-1 text-xs ${
                        t.id === activeId ? 'bg-neutral-950 text-neutral-100' : 'text-neutral-500 hover:text-neutral-300'
                    }`}
            ...
            <button onClick={onNew} className="px-2 py-1 text-xs text-neutral-500 hover:text-neutral-300">
                + Nueva
            </button>
        </div>
    )
