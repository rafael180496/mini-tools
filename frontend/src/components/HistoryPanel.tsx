...
export default function HistoryPanel({entries, loading, error, onRefresh, onClear}: HistoryPanelProps) {
    ...
    function clear() {
        ...
        if (window.confirm(`¿Borrar las ${entries.length} ejecuciones registradas de esta conexión? No se puede deshacer.`)) {
            onClear()
        }
    }
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {!loading &&
                    ...
                    visible.map((e) => {
                        ...
                        return (
                            <div key={e.id} className="border-b border-outline-variant/50">
                                <button
                                    onClick={() => setExpandedId(expanded ? null : e.id)}
                                    title="Click para ver el statement completo y, si falló, el mensaje de error completo"
                                    className="flex w-full items-center gap-2 px-2 py-1 text-left hover:bg-surface-variant"
                                >
